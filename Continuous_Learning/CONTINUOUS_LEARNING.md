...
**Addendum, 2026-09-23 (study SEC1, prompt-log entries 118–120).** This addendum tests this document's own reading of the EQ3 Bayes arm, that the rule's advantage was an advantage over a mis-calibrated Laplace approximation (§7.4). The test calibrates each task's empirical Fisher once, at the task's end, against the curvature the present loss showed along the path travelled, and keeps the Laplace weight 1/2. On the twelve **seen** EQ3 and EQ4 carriers (rung R5, not PASS-0):
- **Tuned λ.** The calibrated Laplace weight is not behind the in-sample-tuned λ on 9 of 12 carriers, against 6 of 12 for the raw Laplace weight and 8 of 12 for the rule at Ω = 1. That is exactly the pre-registered threshold, and it is fragile to the calibration window.
- **The raw gap.** It closes at least half of the raw Laplace gap on 4 of the 6 carriers where that gap was a step or more.
- **The λ spread.** It does not collapse the tuned λ's spread across carriers.
- **mfeat_factors.** It crosses the stability edge: the calibration inherits no step bound (§4.4).
