"""Build Continuous_Learning/Continuous_Learning.pdf from CONTINUOUS_LEARNING.md (a small Markdown subset) with fpdf2.
Supported: '# ', '## ', '### ' headings; paragraphs with **bold**, *italic*, `code`; '- ' bullets; pipe tables; fenced code
...
from matplotlib's bundled set. Run: uv run python Continuous_Learning/build/build_pdf.py
"""
...
MD = DOC_DIR / "CONTINUOUS_LEARNING.md"
OUT = DOC_DIR / "Continuous_Learning.pdf"
...
class Doc(FPDF):
    def __init__(self):
        ...
        self.set_creation_date(dt.datetime(2026, 9, 22, tzinfo=dt.timezone.utc))   # a fixed date so two builds are byte-identical (R9)
        self.set_title("Continuous Learning: the Omega = 1 normalised penalty step"); self.set_author("CRR re-validation repository")

    def footer(self):
        self.set_y(-12); self.set_font("DV", "", 7.5); self.set_text_color(*INK2)
        self.cell(0, 6, f"Continuous Learning — the Ω = 1 rule, CRR re-validation repository, 2026-09-22 — page {self.page_no()}", align="C")
...
def bullets(pdf: Doc, items: list[str]):
    pdf.set_font("DV", "", 9.5)
    pdf.write_html("<ul>" + "".join(f"<li>{inline(i)}</li>" for i in items) + "</ul>", font_family="DV", tag_styles={"code": FontFace(family="DVM")}); pdf.ln(1)
...
def build():
    ...
    while i < n:
        ...
        buf = []
        while i < n and src[i].strip() and not re.match(r"^(#|\||- |> |!\[|```|\$\$)", src[i]): buf.append(src[i].strip()); i += 1
