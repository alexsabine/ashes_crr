"""Build Maps_and_Territories/Maps_and_Territories.pdf from MAPS_AND_TERRITORIES.md (a copy of Rupture_Detection/build/build_pdf.py with the paths and title changed) (a small Markdown subset) with fpdf2.
...
DOC_DIR = ROOT / "Maps_and_Territories"
MD = DOC_DIR / "MAPS_AND_TERRITORIES.md"
OUT = DOC_DIR / "Maps_and_Territories.pdf"
...
class Doc(FPDF):
    def __init__(self):
        ...
        self.set_creation_date(dt.datetime(2026, 9, 24, tzinfo=dt.timezone.utc))   # a fixed date so two builds are byte-identical (R9)
        self.set_title("Maps that change their territory: the empty cut, tested"); self.set_author("CRR re-validation repository")

    def footer(self):
        self.set_y(-12); self.set_font("DV", "", 7.5); self.set_text_color(*INK2)
        self.cell(0, 6, f"Maps that change their territory — CRR re-validation repository, 2026-09-24 — page {self.page_no()}", align="C")
