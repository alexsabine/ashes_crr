...
| `figures/F01…F13.png` | drawn by `build/make_figures.py` from the pinned outputs only; `figures/figures.txt` pins every number placed on them. The document also reuses `AI_Safety/figures/S01, S12, S14` and `Epistemic_Review/figures/E01` |
| `build/make_figures.py`, `build/build_pdf.py` | the figure script (validated palette, one slot per agent) and the PDF builder. The builder is a copy of `AI_Safety/build/build_pdf.py` with nested lists, rule-skipping and markup stripping in boxes and tables |
