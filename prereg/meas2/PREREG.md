# Pre-registration — MEAS2: "change has its own clock" on measles epidemics (re-run of MEAS)

Written: 2026-09-15T21:26Z (container clock). Hash of this folder and
`runs/meas2/frozen/` in `HASH.txt`. Signed tag `prereg-meas2-2026-09-15`.

## Why a second study id, and the status of the data
Study MEAS (`prereg/meas/`, hash 1bd619b1, pushed 21:23:11Z before download)
was **voided by its own format rule**: `runs/meas/results.jsonl` shows the
loader returned 0 series — the R list holds one unnamed empty element before
the 20 data frames, the `rdata` reader raised on it and `pyreadr` cannot read
R lists at all. CLAUDE.md §8: a script changed after the hash voids the study,
so this is a new id. The only change is the loader (skip entries without a
`cases` column; require exactly 20). Hypotheses, thresholds, detector,
sensitivity grid and gate are copied verbatim from MEAS.

Data status, stated exactly: the file was downloaded at 21:23:22Z under MEAS
(sha256 in `data/manifests/meas.sha256`) and is present at this hash. **No
case value has been read**: the MEAS run crashed before reading any column,
and the diagnosis that followed inspected structure only (top-level keys,
entry types, column names, row count 548 × 4). The ledger row will say
`unseen: file present, values unread`. An auditor who disputes that label
should treat this study as confirmatory (seen).
...
(`man/twentymeas.Rd`, read before hashing; the data file was not). Already fetched under MEAS by `data/fetch_measles.py`; sha256 in `data/manifests/meas.sha256`
and echoed in the results header; already listed in `data/SEEN.md`.
Reader: `rdata` (pure Python) with `pyreadr` fallback; both pinned in
`uv.lock`. **Format void rule:** if the reader does not yield exactly 20 series each with a
`cases` column the script exits with FORMAT VOID and the row says so; the
...
## Instrument parameters (all named; frozen in `runs/meas2/frozen/meas2_score.py`)
...
## Hypotheses (scored once by `meas2_score.py score`)
...
## Scoring script
`runs/meas2/frozen/meas2_score.py` (copy of `studies/meas2/meas2_score.py` at hash
