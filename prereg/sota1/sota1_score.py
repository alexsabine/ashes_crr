...
def main():
    ...
    for rule, what in PRED_RULES:
        ...
        # R4 (gate_sota1.txt, G4): nearest class mean read AHEAD of both heads on the near-ceiling surrogate, so these
        # two comparisons are not about CRR and are reported without a verdict
        print(f'  S1-3 pred-{rule:9} [{what}] (same units, seeds 0-4):', fmt(c), '-> report only (AHEAD on the R4 surrogate)')
        rows.append({'id': f'SOTA1-3:pred-{rule}', 'verdict': None, 'cmp': c})
