"""Hypothesis gate (CLAUDE.md R4).
    ...
    uv run python -m crr.surrogates.gate EQB     # EQ-B (bounded present gradient) not behind the tuned weight
    uv run python -m crr.surrogates.gate EQBM    # EQ-B ahead of the registered rule only on the poisoned stream
...
MUST_FAIL = {
    ...
    # EQB (prompt-log entry 74): the bounded rule's not-behind statistic; the same negative controls as EQ2.
    "EQB": {"S-R convex replay, constant label scale (adaptivity idle)",
            "S-X constraint learner, tuned weight a small fraction of present norm",
            "S-Y/LwF softmax MLP, distillation constraint (input scale 4)"},
    # EQBM: the enhancement's mechanism (EQ-B ahead of the registered rule) must be IDLE on clean streams.
    "EQBM": {"S-R convex replay, constant label scale (adaptivity idle)",
             "S-Y softmax MLP, online EWC, 16x Fisher-scale mismatch (input scale 4)"},
...
MUST_PASS = {
    ...
    # EQB must pass where EQ2 passes AND on the poisoned stream; EQBM must pass only on the poisoned stream.
    "EQB": {"S-Y softmax MLP, online EWC, 16x Fisher-scale mismatch (input scale 4)",
            "S-Y softmax MLP, online EWC, scale 4, poisoned batches (x1000, 5 per task)"},
    "EQBM": {"S-Y softmax MLP, online EWC, scale 4, poisoned batches (x1000, 5 per task)"},
...
def gate_EQB(runner, _ev, meta, seeds=range(5), fixed_grid=(0.0625, 0.25, 0.5, 1.0, 2.0, 4.0),
             omega_grid=(0.5, 0.71, 1.0, 1.41, 2.0), margin=0.05):
    """EQB statistic (prompt-log entry 74): EQ-B (the present gradient clipped at kappa x the largest kept length of
    its recent batches, the ratio on the clipped gradient) at Omega = 1 is NOT BEHIND the tuned fixed weight by a
    relative margin on the seed means -- gate_EQ2's comparison with method 'eqb' and the study's mean-based criterion. The registered rule's value at Omega = 1 is printed beside it (not gated here;
    EQBM gates the difference)."""
    seeds = list(seeds)
    fixed_grid = tuple(meta.get("fixed_grid", fixed_grid))
    eqb = {om: np.array([runner("eqb", om, s)["metric"] for s in seeds]) for om in omega_grid}
    eq1 = np.array([runner("eq", 1.0, s)["metric"] for s in seeds])
    wmed = float(np.median([runner("eqb", 1.0, s)["w_med"] for s in seeds])); wm = round(wmed, 3)
    grid = tuple(sorted(set(fixed_grid) | {wm}))
    fx = {w: np.array([runner("fixed", w, s)["metric"] for s in seeds]) for w in grid}
    tuned_w = min(fx, key=lambda w: fx[w].mean())
    # The statistic is on the SEED MEANS, as the EQ3/EQ4 rows score it (mean difference against a step), not gate_EQ2's mean
    # of per-seed ratios: on the poisoned row the tuned weight's error varies by seed over a wide range, and one seed with a
    # small denominator dominates the mean of ratios (AGENT_LOG 63). Per-seed signs are still reported.
    behind = (eqb[1.0].mean() - fx[tuned_w].mean()) / fx[tuned_w].mean()
    per_seed = eqb[1.0] - fx[tuned_w]
    passes = bool(behind <= margin)
    best_om = min(eqb, key=lambda o: eqb[o].mean())
    detail = (f"EQB(Ω=1)={eqb[1.0].mean():.4f} rule(Ω=1)={eq1.mean():.4f} tuned w={tuned_w}:{fx[tuned_w].mean():.4f} "
              f"(w_med of EQB={wmed:.3f}) | behind-by {behind:+.3f} on the seed means (seeds behind: {int((per_seed > 0).sum())}/{len(seeds)}) "
              f"| best Ω={best_om} landscape " + " ".join(f"{o}:{eqb[o].mean():.3f}" for o in omega_grid)
              + f" | reduction arm w={wm}:{fx[wm].mean():.4f}")
    return passes, detail


def gate_EQBM(runner, _ev, meta, seeds=range(5), margin=0.05):
    """EQBM statistic (the enhancement's mechanism): EQ-B at Omega = 1 is AHEAD of the registered rule at Omega = 1
    by >= `margin` relative in the seed mean and in a majority of seeds. Must PASS on the poisoned stream (the
    present gradient is what the poison inflates) and must FAIL on clean streams (the bound must be idle there:
    a PASS on a clean stream would mean EQ-B is a different learner, not a bounded version of the same one)."""
    seeds = list(seeds)
    eq1 = np.array([runner("eq", 1.0, s)["metric"] for s in seeds]); eqb1 = np.array([runner("eqb", 1.0, s)["metric"] for s in seeds])
    rel = (eq1 - eqb1) / np.maximum(eq1, 1e-12)               # >0: EQ-B better (lower metric)
    passes = bool(rel.mean() >= margin and np.mean(rel > 0) > 0.5)
    detail = (f"rule(Ω=1)={eq1.mean():.4f} EQB(Ω=1)={eqb1.mean():.4f} | rel.gain of EQB {rel.mean():+.3f} "
              f"(seeds>0: {int((rel > 0).sum())}/{len(seeds)}) | per seed " + " ".join(f"{r:+.3f}" for r in rel))
    return passes, detail
...
GATES = {"L5": gate_L5, "CUT": gate_CUT, "T1": gate_T1, "EQ": gate_EQ, "EQ2": gate_EQ2, "EQB": gate_EQB, "EQBM": gate_EQBM, "L5R": gate_L5R, "A3": gate_A3, "SAL": gate_SAL}
LEARNER_GATES = {"T1"}
REPLAY_GATES = {"EQ", "EQ2", "EQB", "EQBM"}
SALIENCE_GATES = {"SAL"}
RATE_GATES = {"L5R"}
