"""Study EQ2 — the equanimity rule as a normalised penalty step for online EWC, with ER-sum,
DER++ and LwF as controls (issue #20; prereg/eq2/PREREG.md), on class-incremental streams
built from PMLB datasets absent from data/SEEN.md.

    uv run python studies/eq2/eq2_score.py smoke                  # synthetic data, no download
    uv run python studies/eq2/eq2_score.py all   <dataset> [--out F]   # every arm, seeds 0-4, JSON lines
    uv run python studies/eq2/eq2_score.py score <results.jsonl ...>   # rows EQ2-0..8 as pre-registered
    uv run python studies/eq2/eq2_score.py run   <dataset> <method> <mode> <value> <seed> [--regime std|x4]
...
FALLBACK = ("mfeat_karhunen", "mfeat_zernike", "vowel")          # pre-registered substitutes, in order
...
# ---------------------------------------------------------------- one run
def run(method, mode, value, seed, Xtr, ytr, Xte, yte, K, per_task, smooth=SMOOTH, wcap=WCAP):
    rng = np.random.default_rng(seed)
    d = Xtr.shape[1]; net = MLP(rng, d, K); n = net.flat().size
    ...
    with np.errstate(all="ignore"):
        for task in tasks:
            ...
            theta_task_start = net.flat().copy(); si_omega[:] = 0
    ...
    return dict(method=method, mode=mode, value=value, seed=seed, smooth=smooth, wcap=wcap,
...
def key(o): return (o["method"], o["mode"], round(o["value"], 6), o["smooth"], o["wcap"], o.get("regime", "std"))
...
def run_all(name, data_std, data_x4, K, per_task, meta, out):
    ...
    def go(method, mode, value, sm, cap, regime="std"):
        data = data_std if regime == "std" else data_x4
        for seed in SEEDS:
            o = run(method, mode, value, seed, *data, K, per_task, smooth=sm, wcap=cap)
    ...
    best = max(EWC_COARSE, key=lambda w: np.mean(done[("ewc", "fixed", round(w, 6), SMOOTH, WCAP, "std")]))
    for f in EWC_REFINE:
        w = round(best * f, 6)
...
def score(paths):
    ...
    def A(name, method, mode, value=None, sm=SMOOTH, cap=WCAP, regime="std"):
        v = sorted([r for r in rows if r["dataset"] == name and r["method"] == method and r["mode"] == mode and r["regime"] == regime
                    and (value is None or abs(r["value"] - value) < 1e-6) and r["smooth"] == sm and r["wcap"] == cap], key=lambda r: r["seed"])
        assert len(v) == len(SEEDS), (name, method, mode, value, sm, cap, regime, [r["seed"] for r in v])
        return np.array([r["acc"] for r in v])

    def grid(name, method, regime="std"):
        ws = sorted(set(round(r["value"], 6) for r in rows if r["dataset"] == name and r["method"] == method and r["mode"] == "fixed" and r["regime"] == regime))
        return {w: A(name, method, "fixed", w, regime=regime) for w in ws}

    def fmt(x): return f"{x.mean():.4f} [" + " ".join(f"{q:.2f}" for q in x) + "]"

    print("=" * 112); print("EQ2 scoring — per-seed values in brackets (seeds 0-4); thresholds from prereg/eq2/PREREG.md"); print("=" * 112)
    R = {}
    for name in ds:
        R[name] = {}
    ...
    print(f"EQ2-0 precondition: tuned lambda per carrier {dict(zip(ds, lams))}, span {span:.2f}x -> {'DECIDABLE' if decidable else 'NOT DECIDABLE: tuned lambda did not move >= 10x'}")
    # EQ2-1
    ...
    print(f"EQ2-1 H-EQ2: Ω=1 − tuned per carrier " + ", ".join(f"{d}:{R[d]['d1'].mean():+.4f} (step {R[d]['step']:.2f}; seeds not behind {int((R[d]['d1'] > -R[d]['step']).sum())}/5)" for d in ds)
    ...
    print(f"EQ2-2 reduction: rule − fixed@median per carrier " + ", ".join(f"{d}:{R[d]['eq'][1.0].mean()-R[d]['red'].mean():+.4f}" for d in ds)
    ...
    print(f"EQ2-3 control ER-sum: rule − best fixed " + ", ".join(f"{d}:{R[d]['er_eq'].mean()-R[d]['er_bestv'].mean():+.4f}" for d in ds)
    ...
    print("EQ2-4 control DER++/LwF: best-Ω − tuned " + ", ".join(f"{d}/{m}:{R[d][m][3].mean()-R[d][m][1].mean():+.4f}" for d in ds for m in ("derpp", "lwf")) + f" -> {'holds' if c4 else 'VIOLATED'}")
    # EQ2-5
    print("EQ2-5 diagnostic SI/MAS: Ω=1 − tuned " + ", ".join(f"{d}/{m}:{R[d][m][2].mean()-R[d][m][1].mean():+.4f}" for d in ds for m in ("si", "mas")) + " (report only)")
    # EQ2-6
    best_o = {d: max(OMEGA_GRID, key=lambda o: R[d]["eq"][o].mean()) for d in ds}
    v6 = all(best_o[d] in (0.71, 1.0, 1.41) for d in ds)
    print(f"EQ2-6 Ω plateau: best Ω per carrier {best_o} -> {'PASS' if v6 else 'FAIL'}")
    # EQ2-7
    dom = {m: all((R[d][m].mean() - R[d]["eq"][1.0].mean()) >= R[d]["step"] for d in ds) for m in ("agem", "gradnorm", "mega")}
    print("EQ2-7 published baselines vs rule(Ω=1): " + ", ".join(f"{d}/{m}:{R[d][m].mean()-R[d]['eq'][1.0].mean():+.4f}" for d in ds for m in ("agem", "gradnorm", "mega"))
    ...
    # EQ2-8
    print("EQ2-8 diagnostic x4 regime: Ω=1 − tuned " + ", ".join(f"{d}:{R[d]['x4'][2].mean()-R[d]['x4'][1].mean():+.4f} (tuned {R[d]['x4'][0]:g})" for d in ds) + " (report only)")
...
if __name__ == "__main__":
    ...
    if cmd == "smoke":
        ...
        name = sys.argv[2]
        outp = sys.argv[sys.argv.index("--out") + 1] if "--out" in sys.argv else f"runs/eq2/results_{name}.jsonl"
