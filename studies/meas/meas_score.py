"""Study MEAS — H-L5 ("change has its own clock") on measles epidemics, 20 English
cities, biweekly, 22 years (tsiR `twentymeas`, Grenfell). Unit = city.

    uv run python studies/meas/meas_score.py run                # all cities, all cells -> JSON lines
    uv run python studies/meas/meas_score.py score <results.jsonl>
    uv run python studies/meas/meas_score.py smoke              # synthetic, no data
...
def load_twentymeas(path: Path = RAW):
    ...
    try:
        ...
        for city, df in res.items():
            series[str(city)] = np.asarray(df["cases"], float)
        reader = f"pyreadr (rdata failed: {type(e).__name__})"
...
def score(path):
    ...
    print("=" * 110); print("MEAS scoring — thresholds from prereg/meas/PREREG.md; per-city values shown for the primary cell"); print("=" * 110)
    table = {}
