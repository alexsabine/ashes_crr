...
DATASETS: dict[int, tuple[str, int, int]] = {}     # OpenML data id -> (name, K requested, classes per task); filled from the selection before the hash
...
def score(paths):
    ...
    names = [DATASETS[d][0] for d in DATASETS] if DATASETS else sorted(hdrs)
