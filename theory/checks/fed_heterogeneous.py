...
central model's units). Each node's local world is the central world with every class mean shifted by SHIFT x a standard
normal vector (shared by the nodes: a site whose population differs from the centre's), and the node sees all 10 classes
there. N nodes each start from the anchor and personalise on their local world with the past term
...
Metric: the mean of two final errors (lower is better): RETENTION, the error on the central world's held-out rows of
classes 0-5 (what the anchor knew), and LOCAL, the error on the local world's held-out rows of all 10 classes (what the
node came for); mean over 5 seeds. The shift makes the two pull apart, so the balance between present and past matters.
Also reported: the global knob with EQ-B's clip ('knob+clip'), which separates the clip from the ratio.
...
Statistics (every label computed from the numbers, R15):
    ...
    FED-B      PASS iff on the poisoned node EQ-B is ahead of the registered rule by >= step (gate_EQBM's comparison in
               this setting; the poison wrecks every fixed weight on that node too, so the oracle is not the reference).
               On a row with no poisoned node, FED-B is read on the reference node.
...
FAIL on the clean heterogeneous row (with no poison the bound must be idle). GATE OPEN iff no violation.
    uv run python theory/checks/fed_heterogeneous.py > theory/checks/fed_heterogeneous.txt
"""
...
from crr.surrogates.battery import (_SY_BS, _SY_LR, _SyMLP, _eqb_clip, _sy_ce_grad,  # noqa: E402
                                    EQB_KAPPA)

# ---------------------------------------------------------------- registered constants
D, SEP = 32, 1.4; SHIFT = 1.0; N_PER_CLASS = 400                 # rows per class per world, half held out
...
def _world(rng, mu):
    ...
    for c in range(len(mu)):
        X.append(mu[c] + rng.standard_normal((2 * N_PER_CLASS, D))); y.append(np.full(2 * N_PER_CLASS, c))
    X = np.concatenate(X); y = np.concatenate(y); te = np.zeros(len(y), bool)
    for c in range(len(mu)):
...
def anchor(seed):
    """The shared anchor for one seed: the central and local worlds, the central model trained on classes 0-5 of the
    central world, and the central Fisher."""
    rng = np.random.default_rng(seed)
    mu = rng.standard_normal((10, D)) * SEP; mu_loc = mu + SHIFT * rng.standard_normal((10, D))
    Xtr, ytr, Xte, yte = _world(rng, mu); Ltr, ltr, Lte, lte = _world(rng, mu_loc)
    net = _SyMLP(rng, D)
    ...
    keep = np.isin(yte, CENTRAL_CLASSES)
    return dict(net=net, F=f / N_FISHER * _SY_BS, theta_star=net.flat().copy(), Ltr=Ltr, ltr=ltr, Lte=Lte, lte=lte,
                Rte=Xte[keep], rte=yte[keep])


def node(anc, kind, factor, node_idx, method, value, seed, poison=None):
    """One node's personalisation from the shared anchor. method: 'fixed' (w = value), 'eq' (the registered rule at
    Omega = value), 'eqb' (EQ-B), 'fixedclip' (w = value on EQ-B's clipped present gradient). Returns the metric (mean of
    retention and local error), the two errors and the median derived weight."""
    rng = np.random.default_rng([seed, node_idx, 1])
    net = anc["net"].copy(); F = anc["F"]; ts = anc["theta_star"]
    Ltr, ltr = anc["Ltr"], anc["ltr"]
    unit = factor if kind == "input" else 1.0
    c = factor if kind == "loss" else 1.0
    ii_l = rng.permutation(len(ltr))
    ema_p = ema_q = None; hist = []; wlog = []
    with np.errstate(all="ignore"):
        for step in range(LOCAL_STEPS):
            ii = ii_l[rng.integers(0, len(ii_l), _SY_BS)]
            g_p = c * _sy_ce_grad(net, Ltr[ii] * unit, ltr[ii])
            if poison is not None and POISON_AT <= step < POISON_AT + poison[1]: g_p = poison[0] * g_p
            if method in ("eqb", "fixedclip"): g_p = _eqb_clip(g_p, hist, EQB_KAPPA)
            g_q = 2 * F * (net.flat() - ts)
    ...
    ret = float(net.err(anc["Rte"], anc["rte"])); loc = float(net.err(anc["Lte"] * unit, anc["lte"]))
    return 0.5 * (ret + loc), ret, loc, float(np.median(wlog))
...
def run_row(name, spec, anchors):
    ...
    for i, fac in enumerate(nodes):
        ...
        res.append(dict(fac=fac, fx=fx, eq=np.array([e[0] for e in eq]), eq_w=float(np.median([e[3] for e in eq])),
                        eq_rl=(float(np.mean([e[1] for e in eq])), float(np.mean([e[2] for e in eq]))),
                        eqb=np.array([e[0] for e in eqb]), eqb_w=float(np.median([e[3] for e in eqb])), poisoned=pz is not None, pz=pz))
    ...
    fed1_a, fed1_b = True, False; fedb = None; fb_node = spec["poisoned"] if spec["poisoned"] is not None else ref_i
    for i, r in enumerate(res):
        kc = np.array([node(anchors[s], kind, r["fac"], i, "fixedclip", ref_w, s, r["pz"])[0] for s in SEEDS])
        ...
        if i == fb_node: fedb = bool(r["eq"].mean() - r["eqb"].mean() >= step)
        print(f"   node {i} factor {r['fac']:g}{' (poisoned)' if r['poisoned'] else ''}: oracle w {orc_w:g} err {orc.mean():.4f} (step {step:.4f}) | "
              ...
              f"EQ-B {r['eqb'].mean():.4f} ({d_eqb:+.4f}, w_med {r['eqb_w']:.1f}) | knob+clip {kc.mean():.4f} ({kc.mean() - orc.mean():+.4f})")
        print(f"      rule retention/local error {r['eq_rl'][0]:.4f}/{r['eq_rl'][1]:.4f}")
    ...
    print(f"   FED-B (node {fb_node}): EQ-B ahead of the rule by >= step -> {'PASS' if fedb else 'FAIL'}")
